...
export default function ScannerPage() {
  ...
  const handleAnalyse = async () => {
    ...
    const tid = toast.loading(`Scanning ${captures.length} photo${captures.length>1?"s":""} from ${captures.map(c=>c.angle).join(", ")} angle${captures.length>1?"s":""}...`);
    ...
    try {
      ...
      // Step 2: Send to AI
      setProcessStep(`Analysing ${processed.length} angle${processed.length>1?"s":""} with AI...`);
      const region = profile?.region ?? "Nigeria";
      const parsed = await scanPlantMultiAngle(processed, region, lang, corrections);
      ...
      // Save scan (use first capture's preview for thumbnail)
      const { data:{ user } } = await supabase.auth.getUser();
      if (user) {
  ...
  return (
    <Layout>
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-ink-500 dark:text-gray-400 mb-6">Scan any plant from any angle — crops, weeds, trees</p>
        ...
        {mode==="idle" && (
          <div className="space-y-4">
            ...
            <div className="bg-sky/10 border border-sky/20 rounded-2xl p-4">
              <p className="text-sky font-semibold text-sm mb-2">◈ Multi-Angle Scanning</p>
              <p className="text-ink-500 dark:text-gray-400 text-sm">
                Take up to 4 photos from different angles — front, above, underside, and close-up.
                The AI analyses all photos together for much more accurate identification.
                You can also send just 1 photo if that is all you have.
              </p>
            </div>
        ...
        {mode==="preview" && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {captures.map((cap, i) => (
                <div key={i} className="relative rounded-2xl overflow-hidden border-2 border-terra/30">
                  <img src={cap.preview} alt={cap.angle} className="w-full h-36 object-cover" />
                  <div className="absolute bottom-2 left-2 bg-black/70 text-terra text-xs font-bold px-2 py-1 rounded-lg uppercase tracking-wide">
                    {cap.angle}
                  </div>
                  <button onClick={() => setCaptures(prev => prev.filter((_,j)=>j!==i))}
                    className="absolute top-2 right-2 w-7 h-7 bg-black/60 rounded-full text-white text-xs flex items-center justify-center">✕</button>
                </div>
              ))}
              {/* Add more button */}
              {captures.length < 4 && (
                <button onClick={() => setMode("camera")}
                  className="h-36 rounded-2xl border-2 border-dashed border-terra/30 flex flex-col items-center justify-center gap-2 hover:border-terra transition-colors">
                  <span className="text-terra text-2xl">+</span>
                  <span className="text-ink-500 dark:text-gray-400 text-xs">Add angle</span>
                </button>
              )}
            </div>
            ...
            <button onClick={handleAnalyse}
              ...
              ◈ Scan {captures.length} Photo{captures.length>1?"s":""} {captures.length>1 ? `(${captures.map(c=>c.angle).join(", ")})` : ""}
            </button>
            <button onClick={reset} className="w-full text-ink-500 dark:text-gray-400 h-10 text-sm">Start over</button>
          </div>
        )}
          ...
          <div className="relative rounded-3xl overflow-hidden">
            ...
            <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center">
              ...
              <p className="text-white font-bold" style={{ fontFamily:"'Courier New',monospace",letterSpacing:2 }}>
                ANALYSING {captures.length} ANGLE{captures.length>1?"S":""}
              </p>
        ...
        {mode==="result" && result && (
          <div className="space-y-4">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 to-transparent flex items-end p-5">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {captures.length > 1 && (
                      <span className="text-xs px-2 py-0.5 rounded font-bold bg-sky/80 text-white">
                        {captures.length} ANGLES
                      </span>
                    )}
                  </div>
            ...
            {/* Angles used */}
            {result.angles_used?.length > 0 && (
              <div className="bg-sky/10 border border-sky/20 rounded-2xl p-3 flex gap-3 items-center">
                <p className="text-sky text-sm font-semibold shrink-0">Angles used:</p>
                <div className="flex flex-wrap gap-1">
                  {result.angles_used.map(a => (
                    <span key={a} className="bg-sky/20 text-sky text-xs px-2 py-0.5 rounded-lg font-semibold capitalize">{a}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {result.identification_confidence < 65 && (
              <div className="bg-amber/10 border border-amber/30 rounded-2xl p-4">
                ...
                <p className="text-amber text-xs mt-2">Tip: try taking a clearer photo of the leaf from directly above or below</p>
              </div>
            )}
            ...
            {result.is_weed && result.weed_action && (
              <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-2xl p-4">
                <p className="text-orange-700 dark:text-orange-400 font-bold text-sm mb-1">⚠ Weed Detected</p>
            ...
            {result.is_healthy && (
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-2xl p-4 flex items-center gap-3">
                ...
                <p className="text-green-700 dark:text-green-400 font-semibold">Plant looks healthy. No visible disease or pest detected.</p>
              </div>
            )}
