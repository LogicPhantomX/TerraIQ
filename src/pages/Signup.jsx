      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-dark-surface rounded-3xl p-6 border border-deep-light dark:border-dark-light shadow-card space-y-4">
          ...
          {step === 1 && <>
            ...
            {[["Full name","text","fullName","Emeka Okafor"],["Email","email","email","you@example.com"],["Password","password","password","Minimum 8 characters"]].map(([l,t,k,ph]) => (
              <div key={k}>
                <label className="text-ink-500 dark:text-gray-400 text-sm mb-1.5 block">{l}</label>
                <input type={t} value={form[k]} onChange={e => set(k, e.target.value)} placeholder={ph} className={iClass} />
              </div>
            ))}
            <button
              onClick={() => {
          ...
          {step === 2 && <>
            ...
            <div>
              ...
              <input value={form.farmName} onChange={e => set("farmName", e.target.value)} placeholder="e.g. Okafor Family Farm" className={iClass} />
            </div>
            <div>
