...
export default function ScanHistoryPage() {
  ...
  const filtered = scans.filter(s => {
    ...
    if (filter === "healthy") return s.confidence < 20;
    ...
    <Layout>
      ...
      <div className="flex gap-2 mb-5 overflow-x-auto pb-1">
        {[
          ...
          { key:"healthy", label:`Healthy (${scans.filter(s=>s.confidence<20).length})`          },
        ].map(f => (
          ...
          {filtered.map(scan => (
            ...
            >
              ...
              <div className="p-4 flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    {/* Type badge — no emoji, clean pill */}
                    <span className={`text-xs px-2.5 py-0.5 rounded-lg font-semibold capitalize ${TYPE_STYLE[scan.scan_type] ?? TYPE_STYLE.disease}`}>
                      {scan.scan_type}
                    </span>
                    {/* Severity badge */}
                    <span className={`text-xs px-2.5 py-0.5 rounded-lg font-semibold capitalize ${SEV_STYLE[scan.severity] ?? SEV_STYLE.low}`}>
                      {scan.severity}
                    </span>
                  </div>
                  <p className="text-ink dark:text-white font-semibold text-sm leading-snug">{scan.result}</p>
                ...
                <div className="text-right shrink-0">
                  <div className="w-10 h-10 rounded-xl bg-deep-mid dark:bg-dark-mid flex items-center justify-center">
                    <div className="w-6 h-6 rounded-full border-2 border-terra flex items-center justify-center">
                      <div className="w-2.5 h-2.5 rounded-full bg-terra" style={{ opacity: scan.confidence / 100 }} />
                    </div>
                  </div>
              ...
              {selected?.id === scan.id && plan && (
                <div className="border-t border-deep-light dark:border-dark-light px-4 py-4 space-y-3 bg-deep-mid dark:bg-dark-mid"
