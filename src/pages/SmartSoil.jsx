...
export default function SmartSoilPage() {
  ...
  const analyseImage = async () => {
    if (!imageB64) return;
    setLoading(true);
    const tid = toast.loading("Analysing soil image...");
    try {
      ...
      toast.dismiss(tid);
      toast.success(`Soil rating: ${fullResult.rating?.toUpperCase()}`);
      setMode("result");
  ...
  const runQuestionnaireAnalysis = async (finalAnswers) => {
    setLoading(true);
    const tid = toast.loading("Analysing your farm conditions...");
    try {
  ...
  const runManual = async () => {
    if (!manualForm.nitrogen && !manualForm.organic_carbon) {
      toast.error("Enter at least Total Nitrogen or Organic Carbon");
      return;
    }
    setLoading(true);
    ...
    <Layout>
      ...
      <div className="bg-white dark:bg-dark-surface rounded-2xl border border-deep-light dark:border-dark-light p-6 mb-5 shadow-card space-y-4">
        ...
        {imgPreview && (
          ...
          >
            {loading ? "Analysing soil..." : "Analyse Soil Photo"}
          </button>
        )}
      </div>
