  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log("Weather: Initial coords for region", region, coords);
        ...
        if (city?.trim()) {
          try {
            ...
            console.log("Weather: Geocoding response", gd);
            if (gd.results?.[0]) {
              coords = { lat: gd.results[0].latitude, lon: gd.results[0].longitude };
              displayLoc = `${city}, ${region || "Nigeria"}`;
              console.log("Weather: Using geocoded coords", coords, displayLoc);
            } else {
              console.log("Weather: Geocoding failed, using state coords", coords);
            }
          } catch (geoErr) {
        ...
        console.log("Weather: Set weather data successfully");
      } catch (err) {
