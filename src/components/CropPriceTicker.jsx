...
Return current approximate market prices for these crops in ${region}, Nigeria.
[
  {"crop":"Maize","price":320,"trend":"up","unit":"kg"},
  {"crop":"Cassava","price":185,"trend":"stable","unit":"kg"},
  {"crop":"Tomato","price":480,"trend":"down","unit":"kg"},
...
Prices should be realistic current Naira prices per kg for ${region}.
trend must be: up, down, or stable.`;

  const raw = await askGroq(system, `Give current crop prices for ${region}, Nigeria`, 400);
  try {
...
export default function CropPriceTicker({ region }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await fetchPrices(region ?? "Nigeria");
  ...
  return (
    <div className="bg-white dark:bg-dark-surface rounded-2xl border border-deep-light dark:border-dark-light shadow-card overflow-hidden">
      <div className="px-5 py-4 border-b border-deep-light dark:border-dark-light flex justify-between items-center">
        ...
        <span className="text-ink-500 dark:text-gray-400 text-xs">{region ?? "Nigeria"} · per kg</span>
      </div>
